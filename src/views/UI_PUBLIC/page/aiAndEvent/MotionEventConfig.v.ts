 ...
 * @LastEditTime: 2024-08-21 14:56:25
 */
...
export default defineComponent({
    ...
    setup() {
                ...
                for (let i = 0; i < tableData.value.length; i++) {
                    ...
                    queryMotion(sendXml).then((res: any) => {
                        ...
                        if (res('status').text() == 'success') {
                            ...
                            row.schedule = {
                                value: res('//content/chl/trigger/triggerSchedule/schedule').attr('id'),
        ...
        const recordCloseAll = () => {
            pageData.value.recordChosedListAll = []
            pageData.value.recordChosedIdsAll = []
            // console.log('recordChosedListAll', pageData.value.recordChosedListAll)
            // console.log('recordChosedIdsAll', pageData.value.recordChosedIdsAll)
            // console.log('tableData', tableData.value)
            pageData.value.recordIsShowAll = false
            recordRef.value.handleClose()
        }
        const setRecord = function (index: number) {
            ...
            // console.log('triggerDialogIndex', pageData.value.triggerDialogIndex)
            // console.log('tableData', tableData.value)
        }
        const recordConfirm = (e: { value: string; label: string }[]) => {
            // console.log('recordConfirm', e)
        ...
        const snapCloseAll = () => {
            pageData.value.snapChosedListAll = []
            pageData.value.snapChosedIdsAll = []
            // console.log('snapChosedListAll', pageData.value.snapChosedListAll)
            // console.log('snapChosedIdsAll', pageData.value.snapChosedIdsAll)
            // console.log('tableData', tableData.value)
            pageData.value.snapIsShowAll = false
            snapRef.value.handleClose()
        }
        ...
        const alarmOutCloseAll = () => {
            pageData.value.alarmOutChosedListAll = []
            pageData.value.alarmOutChosedIdsAll = []
            // console.log('alarmOutChosedListAll', pageData.value.alarmOutChosedListAll)
            // console.log('alarmOutChosedIdsAll', pageData.value.alarmOutChosedIdsAll)
            // console.log('tableData', tableData.value)
            pageData.value.alarmOutIsShowAll = false
            alarmOutRef.value.handleClose()
        }
        ...
        const addEditRow = function (row: MotionEventConfig) {
            ...
            console.log('applyDisabled', pageData.value.applyDisable)
        }
        const getSavaData = function (rowData: MotionEventConfig) {
            ...
            const schedule = rowData.schedule ? true : false
            sendXml += `
                        ...
                        <triggerSchedule>
                            ...
                            <schedule id="${rowData.schedule.value}"></schedule>
                        </triggerSchedule>
